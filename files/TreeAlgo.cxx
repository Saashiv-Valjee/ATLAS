...
EL::StatusCode TreeAlgo :: execute ()
{
  ...
  for(const auto& systName: event_systNames){
    ...
    if (!m_truthMetContainerName.empty() )      { helpTree->AddTruthMET(m_truthMetDetailStr, m_truthMetContainerName);              }
  ...
  for(const auto& systName: event_systNames){
    ...
    */
    ...
    if ( !m_truthMetContainerName.empty() ) {
      if ( !HelperFunctions::isAvailable<xAOD::MissingETContainer>(m_truthMetContainerName/*.at(ll)*/, m_event, m_store, msg()) ) {continue;}

        const xAOD::MissingETContainer* inTruthMetCont = 0;
        ANA_CHECK( HelperFunctions::retrieve(inTruthMetCont, m_truthMetContainerName, m_event, m_store, msg()) );
        helpTree->FillTruthMET( inTruthMetCont, m_truthMetContainerName);
   }
