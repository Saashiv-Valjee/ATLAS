...
HelpTreeBase::~HelpTreeBase() {
    ...
    //truthMet
    for (auto truth_met: m_TruthMet)
      delete truth_met.second;
...
void HelpTreeBase::AddTruthMET(const std::string& detailStr, const std::string& truthMetName)
{
  if(m_debug) Info("AddTruthMET()", "Adding TruthMET %s with variables: %s", truthMetName.c_str(), detailStr.c_str());
  m_TruthMet[truthMetName] = new xAH::TruthMET(truthMetName, detailStr, m_units);
  m_TruthMet[truthMetName]->m_debug = m_debug;

  xAH::TruthMET* thisTruthMet = m_TruthMet[truthMetName];

  thisTruthMet->setBranches(m_tree);
  this->AddTruthMETUser(truthMetName, detailStr);
}

void HelpTreeBase::FillTruthMET( const xAOD::MissingETContainer* truthMet, const std::string& truthMetName)

{
  this->ClearTruthMET(truthMetName);

  xAH::TruthMET* thisTruthMet = m_TruthMet[truthMetName];
  thisTruthMet->FillTruthMET(truthMet);

  this->FillTruthMETUser(truthMet, truthMetName);
  return;
}

void HelpTreeBase::ClearTruthMET(const std::string& truthMetName) {

  xAH::TruthMET* thisTruthMet = m_TruthMet[truthMetName];
  thisTruthMet->clear();
  this->ClearTruthMETUser(truthMetName);
}
