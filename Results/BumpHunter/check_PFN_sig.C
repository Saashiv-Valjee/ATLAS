...
for (int i = 515487; i < 515527; i++){
	TString file_path=Form("FILE/PFN/SR/v8p1_PFNv6_%d_SR.root",i);
	...
	hist->SetTitle(Form("v8p1_PFNv6_%d SR log plot",i));
	hist->Draw("E0");
	gPad->SetLogy(1);
	txt->Draw();
	cv_log.Print(Form("results/PFN/check/SR/v8p1_PFNv6_%d_SR_log.png",i));

	TCanvas cv("cv","cv",800,600);
	hist->SetTitle(Form("v8p1_PFNv6_%d SR plot",i));
	gPad->SetLogy(0);
	gStyle->SetOptStat(0);
	hist->Draw("E0");
	txt->Draw();
	cv.Print(Form("results/PFN/check/SR/v8p1_PFNv6_%d_SR.png",i));
	delete txt;
	delete hist;
}
