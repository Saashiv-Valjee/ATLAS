...
string dir_data = "/eos/atlas/atlascerngroupdisk/phys-exotics/jdm/svjets-schannel/v3/v3.6";
string outfile_path = "/eos/user/e/ebusch/SVJ/Plots";
Color_t mycolors[12] = { kBlack, kBlack, kBlue, kGreen, kMagenta, kCyan, kOrange, kMagenta+1, kAzure+7, kOrange };
...
vector<TH1F*> Get1DHists(vector<string> inputFiles, string branch, PlotParams myParams){
  ...
  for (auto inputFile: inputFiles){
    ...
    tree0->Draw(Form("%s >> h", branch.c_str()), "weight");
...
void simplePlotter(){
  ...
  vector<string> inputFiles = {"user.ebusch.QCDbkg_low.root", "user.ebusch.QCDbkg_high.root","user.ebusch.508547.root", "user.ebusch.508548.root", "user.ebusch.508549.root", "user.ebusch.508550.root"};
  vector<string> leg_names = {"QCD", "Z': 1500, r: 0.8", "Z': 1500, r: 0.3", "Z': 750, r: 0.8", "Z': 750, r: 0.3"};
  ...
  */
  ...
  vector<TH1F*> jet1_pt;
  jet1_pt = Get1DHists(inputFiles, "jet1_pt", P_jet1_pt);
  ...
  Plot1DHists(jet1_pt_combined, leg_names, "jet1_pt"); 
