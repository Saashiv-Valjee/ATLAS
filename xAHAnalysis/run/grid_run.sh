...
configFile=$WorkDir_DIR/src/SVJAlgo/data/config_SVJAlgo_QCD.py
sigconfigFile=$WorkDir_DIR/src/SVJAlgo/data/config_SVJAlgo_signal.py

#for BKG in ttbar wplusjets zplusjets
for BKG in multijet ttbar wplusjets zplusjets
do
	submitdir=grid_${BKG}
...
do
	...
	do
		...
		xAH_run.py --inputRucio --inputList --files $inputFile --config $sigconfigFile --extraOptions="--doFatJet" --force --submitDir $submitdir prun --optBatchShellInit 'source $WorkDir_DIR/setup.sh' --optGridOutputSampleName=user.ebusch.%in:name[2]%.%in:name[3]%.v5a.${MC} --optGridNGBPerJob=2
	done
done
