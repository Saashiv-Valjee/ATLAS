...
namespace CP {
      ...
      m_appliedSystematics(0)
  {
    ...
    //declareProperty("MaskedRegionsMap", m_bd_dead_mapFile="JetTileCorrection/Tile_maskedDB_data16_fromChristos.conf", "Masked regions DB file");
    declareProperty("MaskedRegionsMap", m_bd_dead_mapFile="JetTileCorrection/Tile_maskedDB_Run2_fromChristos.conf", "Masked regions DB file");
  ...
  StatusCode JetTileCorrectionTool :: initialize() {
    ...
    //std::cout << "Set up JetTileCorrectionTool -- this is MC? " << m_isMC << std::endl;
    //set RJET
    setRJET( (float) RJET );
  ...
  void JetTileCorrectionTool :: loadDeadDB(){
    ...
    if(m_isMC){
      ...
      //std::cout << "The file used for tile correction = " << mapFilename << std::endl;
      std::ifstream mapFile;
      mapFile.open(mapFilename);
     
      std::string line;
      while (std::getline(mapFile, line)){
	//std::cout << "Lines from the map file = " << line << std::endl;
	if(line[0]=='#') continue;
	
	std::istringstream iss(line);
	int part,mod, irun, erun, ilb, elb;
	std::string modname;
	
	if (!(iss >> part >> mod >> irun >> erun >> ilb >> elb >> modname)) { break; } // error
	
	//std::cout << "part = " << part << ", mod = " << mod << ", irun = " << irun << ", erun = " << erun << ", ilb = " << ilb << ", elb = " << elb << ", modname = " << modname << std::endl;

	Hole rdead = partModToHole(part, mod);
	rdead.iov = make_pair(irun,erun);
	rdead.iov_lb = make_pair(ilb, elb);
	dbholes.push_back( rdead );  
    ...
    int dbh=1;
    //std::cout << "size of dbholes = " << dbholes.size() << std::endl;
	...
	m_db_dead_LB[Form("DB%d",dbh)] = h;
      ...
      //std::cout << "Adding DB dead module at (eta1,phi1)=(" << h.eta1 << "," << h.phi1 << ")" << std::endl;
  ...
  void JetTileCorrectionTool :: loadModulesFromMap(const xAOD::Jet& jet, JTC::TS &status, std::map<std::string,Hole> hmap, PART part, TYPE type){
    ...
    for (const auto& mod : hmap){
      ...
      if(!m_isMC && type == TYPE::DB && !inIOV(mod.second, m_current_run, m_current_lb)) continue;
      std::cout << "I am getting here:: common IOV between map and current run" << std::endl;
  ...
  StatusCode JetTileCorrectionTool :: loadAllModules(const xAOD::Jet& jet, JTC::TS &status){
	...
	return StatusCode::FAILURE;
      ...
      m_current_lb = ei->lumiBlock();
      //std::cout << "current run number = " << m_current_run << std::endl;
      //std::cout << "current lumi block = " << ei->lumiBlock() << std::endl;
    }
    ...
    //std::cout << "size of position masked = " << m_position_masked.size() << std::endl;
  ...
  bool JetTileCorrectionTool :: inIOV(Hole region, int run, int lb){
    if( region.iov.first>=0 &&  (run < region.iov.first) ) return false;
    if( region.iov.second>=0 &&  (run > region.iov.second) ) return false;

    if(region.iov.first>=0 && region.iov.second>=0 && (run >= region.iov.first) && (run <= region.iov.second) ){
      if( region.iov_lb.first>=0 &&  (lb < region.iov_lb.first) ) return false;
      if( region.iov_lb.second>=0 &&  (lb > region.iov_lb.second) ) return false;
    }
    std::cout << "Found a match " << std::endl;
    std::cout << "Hole Run range = " << region.iov.first << " - " << region.iov.second << std::endl;
    std::cout << "Hole lumi range = " << region.iov_lb.first << " - " << region.iov_lb.second << std::endl;
    std::cout << "current run = " << run << ", current lumi = " << lb << std::endl;
    return true;
  }
